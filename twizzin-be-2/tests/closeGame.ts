...
export async function closeGame(
  ...
  // After declaring winners, let's test close game error cases
  console.log('Testing close game error cases...');

  // Case 1: Non-winner tries to claim (needed for close game test setup)
  try {
  ...
  // Case 2: Double claim attempt (needed for close game test setup)
  ...
  console.log('All close game tests completed successfully');

  // Final close game scenarios
  console.log('\nTesting additional close game scenarios');

  // Create a new game just for close testing
  ...
  // Verify player has received funds
  ...
  // Player joins and submits answers
  ...
  // Declare winner
  await executeDeclareWinners(gameCode4, [player4.publicKey], [playerPda4]);

  // Test 1: Try to close game before winner claims (should fail)
  ...
  // Have winner claim prize (required before closing)
  ...
  // Test 2: Try to close already closed game (should fail)
